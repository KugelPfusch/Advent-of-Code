...
function playBingo(bingonumbers, arr){
    ...
    let value = 0;
    let bingoArr = new Array(arr.length).fill(false);
            ...
            for(let k = 0; k < arr[j].length; k++){
                ...
                for(let l = 0; l < arr[j][k].length; l++){
                    ...
                    if(arr[j][k][l] === bingonumbers[i]){
                        ...
                        value = bingoSuccessfull(boolArray, arr, bingoArr);
                        if(value){
                            bingoArr[j] = true;
                            if(bingoArr.filter(e => e === true).length === arr.length){
...
function bingoSuccessfull(boolArray, arr, bingoArray){
    let valrow = 0;
    let valcol = 0;
    let countrow = 0;
    let countcolumn = 0;
